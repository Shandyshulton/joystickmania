...
class AdminFlowTest extends TestCase
{
    ...
    public function test_admin_can_access_dashboard(): void
    {
        ...
        $this->actingAs($admin, 'admin')->get('/admin/rentals')->assertOk();
        $this->actingAs($admin, 'admin')->get('/admin/memberships')->assertOk();
        $this->actingAs($admin, 'admin')->get('/admin/members')->assertOk();
        $this->actingAs($admin, 'admin')->get('/admin/rooms')->assertOk();
        $this->actingAs($admin, 'admin')->get('/admin/units')->assertOk();
        $this->actingAs($admin, 'admin')->get('/admin/tiers')->assertOk();
    }
    ...
    public function test_admin_confirming_booking_writes_activity_log(): void
    {
        ...
        $response = $this->actingAs($admin, 'admin')->patch('/admin/bookings/'.$booking->id, [
    ...
    public function test_admin_confirming_membership_activates_tier_and_sets_valid_until(): void
    {
        ...
        $response = $this->actingAs($admin, 'admin')->patch('/admin/memberships/'.$purchase->id, [
            'payment_status' => 'sudah_bayar',
    ...
    public function test_admin_can_manage_rooms_crud(): void
    {
        ...
        $response = $this->actingAs($admin, 'admin')->post('/admin/rooms', [
            'nama_room' => 'Room Baru',
            'kapasitas' => 4,
            'konsol_tersedia' => ['PS5'],
            'harga_per_jam' => 60000,
    ...
    public function test_rejected_payment_forces_booking_status_cancelled(): void
    {
        ...
        $this->actingAs($admin, 'admin')->patch('/admin/bookings/'.$booking->id, [
    ...
    public function test_staff_with_permission_can_access_allowed_menu_only(): void
    {
        ...
        $this->actingAs($staff, 'admin')->get('/admin/bookings')->assertOk();

        // Menu yang tidak dicentang -> 403
        $this->actingAs($staff, 'admin')->get('/admin/rentals')->assertForbidden();
        $this->actingAs($staff, 'admin')->get('/admin/rooms')->assertForbidden();
        // Manajemen user khusus super admin -> 403
        $this->actingAs($staff, 'admin')->get('/admin/users')->assertForbidden();
    }
    ...
    public function test_super_admin_can_manage_users(): void
    {
        ...
        $response->assertSessionHasNoErrors();
        $this->assertDatabaseHas('admins', ['email' => 'staffbaru@example.com', 'role' => 'staff']);
    }
    ...
    public function test_admin_cannot_access_user_management(): void
    {
        ...
        $this->actingAs($admin, 'admin')->get('/admin/users')->assertForbidden();
    }
}
